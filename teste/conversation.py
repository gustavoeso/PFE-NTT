...
def multi_table_search(buyer_request: str) -> str:
    ...
    # Step 4: check in loja_{store_info['numero']}
...
#####################################################################
# 6) BuyerChain, SellerChain, ControllerChain
#####################################################################
...
def should_stop_conversation() -> bool:
    """
    Returns True if the buyer explicitly said "Vou levar" or "Não vou levar".
    """
...
# -----------------------------------------------------------------
# NOTE: We have REMOVED the `main()` loop here. This file is now
# just a "library" that provides DB logic and chain objects for
# the FastAPI server to import.
# -----------------------------------------------------------------
